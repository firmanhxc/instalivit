...
class Easy_Instagram_Utils {
	...
	private function get_thumbnail_html_image( $element ) {
		...
		switch ( $element['thumbnail_click'] ) {
			...
			case 'colorbox':
				// $link = '<a class="example6" href="http://localhost/wordpress/">Outside Webpage (Iframe)</a>';
				$link = '<a class="example6" href="http://localhost/wordpress/wp-content/plugins/easy-instagram/include/class-easy-instagram-bandung.php?media_id='.$element['media_id'].'&token='.$access_token.'" >';
				// $link = '<a class="example6" onclick="PopupRateComment(12345,09090909090)" >';

				// $link = sprintf( '<a href="%s" class="example6" rel="%s" title="%s">', $thumbnail_link_url, $unique_rel ,$thumbnail_link_title );
				$out .= apply_filters( 'easy_instagram_thumb_link', $link );
				$has_thumb_action = true;
				break;
