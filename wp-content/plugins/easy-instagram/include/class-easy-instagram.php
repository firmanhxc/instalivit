...
class Easy_Instagram {
	...
	public function __construct() {
		...
		$this->defaults = array(
			...
			'author_text' => __( '%s', 'Easy_Instagram' ),
			'thumb_click' => 'colorbox',
	...
	public function searchForUsername($id, $array) {
...
// echo $endpoint_type."_0000";
// die();

// echo $instagram."<br>";
// echo $endpoint_id."<br>";
// echo $limit."<br>";
// echo $endpoint_type."<br>";
// die();
	...
	public function _get_render_elements_for_ajax( $args ) {
		...
		if($args['user'] != ""){
			if (strpos($args['user'], ',')){
				$explode_user = explode(',', $args['user']);
				if(count($explode_user) > 0){
					foreach ($explode_user as $key_ex => $val_ex) {
						$data_search['user'][$key_ex] = $val_ex;
						$json = file_get_contents('https://api.instagram.com/v1/users/search?q='.$val_ex.'&access_token='.$access_token);
						$json_array = json_decode($json);
						if(count($json_array->data) > 0){
							foreach ($json_array->data as $key => $val) {
							   if ($val->username === $val_ex) {
									$data_search['user_id'][$key_ex] = $val->id;
							   }
							}
						}
					}
				...
				$data_search['user'][0] = $args['user'];
				$json = file_get_contents('https://api.instagram.com/v1/users/search?q='.$args['user'].'&access_token='.$access_token);
				$json_array = json_decode($json);
				if(count($json_array->data) > 0){
					foreach ($json_array->data as $key => $val) {
					   if ($val->username === $args['user']) {
							$data_search['user_id'][$key] = $val->id;
							// $data_search['user'][0] = $val;
					   }
					}
				}
			}						
		...
		foreach ($data_search['user_id'] as $key => $val_search) {
			...
			$out .= '<input type="hidden" name="user_id" value="' . esc_attr( $val_search ) .'" />';
	...
	private function _get_render_elements( $instagram_elements, $args ) {
		...
		foreach ( $instagram_elements as $elem ) {
			...
			$current_template_element['media_id'] = $media_id;
			$current_template_element['access_token'] = $access_token;
	...
	public function generate_content( $params ) {
		...
		if ( empty( $tag ) && empty( $user_id ) && empty( $user )) {
			return '';
		}
